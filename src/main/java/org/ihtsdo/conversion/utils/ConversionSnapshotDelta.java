...
public class ConversionSnapshotDelta {
	...
	public static boolean snapshotFile(File file, File tempSortingFolder,File tempSortedFinalfolder, File resultFile, String date, int[] sortColumns, int snapshotIndex, int effTimeColumn) {
		try {
			...
			if (!sorted) {
				...
				fsc = null;
				System.gc();
				SnapshotGenerator sg = new SnapshotGenerator(sortedFile, date, snapshotIndex, effTimeColumn, resultFile, null, null);
				sg.execute();
				sg=null;
				System.gc();
				...
				SnapshotGenerator sg = new SnapshotGenerator(file, date, snapshotIndex, effTimeColumn, resultFile, null, null);
				sg.execute();
				sg=null;
				System.gc();
			}
